...
**AETHER** is a high-performance, open-source image generation engine optimized for **Metal (macOS)** and **CUDA (Linux)**. It represents the peak of performance and infinite creativity.d version of Z-Image that matches or exceeds leading competitors with only **8 NFEs** (Number of Function Evaluations). It offers **⚡️sub-second inference latency⚡️** on enterprise-grade H800 GPUs and fits comfortably within **16G VRAM consumer devices**. It excels in photorealistic image generation, bilingual text rendering (English & Chinese), and robust instruction adherence.
...
- [Google Cloud Run](./docs/cloud_run.md) | [中文指南](./docs/cloud_run_zh.md)
...
You can run AETHER using Docker. This is useful for deployment on Linux servers or for keeping your environment clean.

### Prerequisites
- Docker and Docker Compose installed.
- Model files downloaded to the project root.
...
> **Note**:
> - The current directory is mounted to `/app` in the container, so your local model files are accessible.
> - On macOS, Docker does **not** support Metal acceleration. Performance will be significantly slower (CPU only) compared to running natively.
> - On Linux with NVIDIA GPUs, you may need to configure the [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/install-guide.html) and update `docker-compose.yml` to enable GPU support.
